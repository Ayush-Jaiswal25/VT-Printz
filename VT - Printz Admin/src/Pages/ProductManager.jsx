...
const ProductManager = () => {
    ...
    const fetchServices = async (catId) => {
        setLoading(true);
        try {
            const res = await axios.get(`${API_BASE}/services?categoryId=${catId}`);
    ...
    const fetchProducts = async (servId) => {
        setLoading(true);
        try {
            const res = await axios.get(`${API_BASE}/products?serviceId=${servId}`);
    ...
    return (
        <div className="bg-white rounded-xl border p-6 shadow-sm min-h-[500px]">
            {/* Header / Breadcrumbs */}
            <div className="flex items-center justify-between mb-6 border-b pb-4">
                <div className="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <span
                        className={`cursor-pointer ${view === 'categories' ? 'font-bold text-black' : ''}`}
                        onClick={() => { setView('categories'); setBreadcrumbs([]); setSelectedCategory(null); setSelectedService(null); fetchCategories(); }}
                    >
                        Categories
                    </span>
                    {selectedCategory && (
